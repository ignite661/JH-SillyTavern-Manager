...
clear
echo -e "${CYAN}欢迎使用 纪贺的SillyTavern一键安装器！${NC}"
...
proot-distro exec ubuntu -- /bin/bash -c "curl -s -L -o /root/jh_manager.sh ${MANAGER_SCRIPT_URL} && chmod +x /root/jh_manager.sh"
...
# 创建自启标志文件
proot-distro exec ubuntu -- touch "$AUTOSTART_FLAG_FILE"
